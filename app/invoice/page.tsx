...
export default function InvoicePage({ editId, isViewOnly, onBack }: DocumentEditorProps) {
  return (
    <Suspense fallback={<div className="flex h-screen items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-[#E8973A]" /></div>}>
      <InvoiceEditor propEditId={editId} propIsViewOnly={isViewOnly} onBack={onBack} />
    </Suspense>
  );
}

function InvoiceEditor({ propEditId, propIsViewOnly, onBack }: { propEditId?: string; propIsViewOnly?: boolean; onBack?: () => void }) {
  ...
  const handleInvoiceTypeChange = (type: "normal" | "cash") => {
    ...
    if (type === "cash") {
      ...
      if (invoiceNo.startsWith("# ") && !invoiceNo.startsWith("# C")) {
        setInvoiceNo(invoiceNo.replace("# ", "# C"));
      }
    } else {
      setTitle("INVOICE");
  ...
  const loadExistingDocument = async (id: string) => {
    setIsLoading(true);
    try {
      const doc: any = await getDocumentFromFirestore("invoice", id);

      if (doc) {
        setTitle(doc.title || "INVOICE");
        ...
        setDueDate(doc.dueDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]);
        ...
        setInvoiceType(doc.invoiceType || "normal");
  ...
  const saveCurrentInvoice = async () => {
    ...
    try {
      if (documentId) {
        await updateInvoiceInFirestore(documentId, docData);
      } else {
        const newId = await saveInvoiceToFirestore(docData);
        setDocumentId(newId);
      }
    } catch (err) {
        ...
        <div className="mx-auto w-full max-w-7xl px-2 sm:px-4 md:px-8">
          ...
          <div className="mt-8 lg:hidden px-2">
            <button
              type="button"
              onClick={() => setShowPreview(true)}
              className="w-full bg-[#E8973A] hover:bg-[#d4832b] text-white font-semibold py-3.5 px-4 rounded-xl transition shadow-lg shadow-[#E8973A]/20 active:scale-[0.98] text-base flex justify-center items-center font-bold"
            >
              Preview & Export
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-center items-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[92vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="p-4 bg-gray-50 border-b border-gray-150 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
              <div>
                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">PDF Export Preview</span>
